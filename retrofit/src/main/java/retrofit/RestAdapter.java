...
/** 
 * Adapts a Java interface to a REST API. <p> API endpoints are defined as methods on an interface with annotations providing metadata about the form in which the HTTP call should be made. <p> The relative path for a given method is obtained from an annotation on the method describing the request type. The built-in methods are   {@link retrofit.http.GET GET},  {@link retrofit.http.PUT PUT},   {@link retrofit.http.POST POST},   {@link retrofit.http.HEAD HEAD}, and   {@link retrofit.http.DELETE DELETE}. You can define your own HTTP method by creating an annotation that takes a {code String} value and itself is annotated with  {@link retrofit.http.RestMethod @RestMethod}. <p> Method parameters can be used to replace parts of the URL by annotating them with  {@link retrofit.http.Path @Path}. Replacement sections are denoted by an identifier surrounded by curly braces (e.g., "{foo}"). To add items to the query string of a URL use  {@link retrofit.http.Query @Query}. If the path or query element has already been URI encoded use   {@link retrofit.http.EncodedPath @EncodedPath} or {@link retrofit.http.EncodedQuery}
 * @EncodedQuery } to prevent repeated encoding. <p> HTTP requests happen in one of two ways: <ul> <li>On the provided HTTP  {@link Executor} with callbacks marshaled to the callback{@link Executor}. The last method parameter should be of type   {@link Callback}. The HTTP response will be converted to the callback's parameter type using the specified  {@link retrofit.converter.Converter Converter}. If the callback parameter type uses a wildcard, the lower bound will be used as the conversion type. <li>On the current thread returning the response or throwing a   {@link RetrofitError}. The HTTP response will be converted to the method's return type using the specified  {@link retrofit.converter.Converter Converter}. </ul> <p> The body of a request is denoted by the   {@link retrofit.http.Body @Body} annotation. The objectwill be converted to request representation by a call to {@link retrofit.converter.Converter#toBody(Object) toBody} on the supplied{@link retrofit.converter.Converter Converter} for this instance. The body can also be a{@link TypedOutput} where it will be used directly.<p> Alternative request body formats are supported by method annotations and corresponding parameter annotations: <ul> <li> {@link retrofit.http.FormUrlEncoded @FormUrlEncoded} - Form-encoded data with key-valuepairs specified by the  {@link retrofit.http.Field @Field} parameter annotation.<li> {@link retrofit.http.Multipart @Multipart} - RFC 2387-compliant multi-part data with partsspecified by the  {@link retrofit.http.Part @Part} parameter annotation.</ul> <p> Additional static headers can be added for an endpoint using the {@link retrofit.http.Headers @Headers} method annotation. For per-request control over a headerannotate a parameter with  {@link Header @Header}. <p> For example: <pre> public interface MyApi { &#64;POST("/category/{cat}") // Asynchronous execution. void categoryList(@Path("cat") String a, @Query("page") int b, Callback&lt;List&lt;Item>> cb); &#64;POST("/category/{cat}") // Synchronous execution. List&lt;Item> categoryList(@Path("cat") String a, @Query("page") int b); } </pre> <p> Calling   {@link #create(Class)} with {@code MyApi.class} will validate and create a newimplementation of the API.
 * @author Bob Lee (bob@squareup.com)
 * @author Jake Wharton (jw@squareup.com)
 */
public class RestAdapter {
  public static int LOG_CHUNK_SIZE=4000;
  public static String THREAD_PREFIX="Retrofit-";
  public static String IDLE_THREAD_NAME=THREAD_PREFIX + "Idle";
  /** 
 ...
 */
  ...
  public RestAdapter(  Server server,  Client.Provider clientProvider,  Executor httpExecutor,  Executor callbackExecutor,  RequestInterceptor requestInterceptor,  Converter converter,  Profiler profiler,  ErrorHandler errorHandler,  Log log,  LogLevel logLevel){
    this.server=server;
 ...
 */
  @SuppressWarnings("unchecked") public <T>T create(  Class<T> service){
...
public class RestHandler implements InvocationHandler {
    public Map<Method,RestMethodInfo> methodDetailsCache=new LinkedHashMap<Method,RestMethodInfo>();
 ...
 */
    public Object invokeRequest(    RequestInterceptor requestInterceptor,    RestMethodInfo methodDetails,    Object[] args){
      methodDetails.init();
 ...
 */
  public Request logAndReplaceRequest(  Request request) throws IOException {
 ...
 */
  public Response logAndReplaceResponse(  String url,  Response response,  long elapsedTime) throws IOException {
 ...
 */
  ...
  /** 
 * Build a new   {@link RestAdapter}. <p> Calling the following methods is required before calling   {@link #build()}: <ul> <li>  {@link #setServer(Server)}</li> <li>  {@link #setClient(Client.Provider)}</li> <li>  {@link #setConverter(Converter)}</li> </ul> <p> If you are using asynchronous execution (i.e., with   {@link Callback Callbacks}) the following is also required: <ul> <li>  {@link #setExecutors(java.util.concurrent.Executor,java.util.concurrent.Executor)}</li> </ul>
 */
 ...
 * @param callbackExecutor Executor on which any {@link Callback} methods will be invoked. Ifthis argument is  {@code null} then callback methods will be run on the same thread as theHTTP client.
 */
 ...
 */
    public RestAdapter build(){
      ...
      ensureSaneDefaults();
      return new RestAdapter(server,clientProvider,httpExecutor,callbackExecutor,requestInterceptor,converter,profiler,errorHandler,log,logLevel);
    }
